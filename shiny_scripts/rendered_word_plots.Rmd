...
```{r}
title_map <- c(
  shap_long_micom      = "MERF Micom",
  shap_long_basic      = "MERF Basic",
  shap_long_all      = "MERF Combined",
  shap_long_all_no_age_sex      = "MERF Combined (-)",
  shap_long_all_no_clin      = "MERF Combined (-c)",
  shap_long_metabo   = "MERF Metabolomics",
  shap_long_grs      = "MERF GRS",
  shap_long_pathway  = "MERF Pathway",
  shap_long_taxa     = "MERF Taxonomy",
  shap_long_meta     = "MERF Clinical",
...
# Save large PDF
ggsave(
  filename = file.path(omic_out_dir, "shap_02/all_shap_plots_combined_abs.pdf"),
...
```{r}
title_map_delta <- c(
  shap_delta_micom    = "MERF Micom",
  shap_delta_basic    = "MERF Basic",
  shap_delta_all      = "MERF Combined",
  shap_delta_all_no_age_sex      = "MERF Combined (-)",
  shap_delta_all_no_clin      = "MERF Combined (-c)",
  shap_delta_metabo   = "MERF Metabolomics",
  shap_delta_grs      = "MERF GRS",
  shap_delta_pathway  = "MERF Pathway",
  shap_delta_taxa     = "MERF Taxonomy",
  shap_delta_meta     = "MERF Clinical",
...
# Save large PDF
ggsave(
  filename = file.path(omic_out_dir, "shap_02/all_delta_shap_plots_combined_abs.pdf"),
...
for (omic_name in names(all_r2_dfs)) {
  ...
  sig_plot <- ggplot(r2_annotated, aes(x = Model, y = R2m, fill = Model)) +
    ...
    labs(title = NULL,
         x = NULL,
         y = "Marginal R\u00B2") +
    combined_plot_theme(base_size = 22)
...
gtsave(final_anova_table_initial, 
       filename = paste0(omic_out_dir, "/anova_tables_02/anova_initial_table_with_grs.png"))
```
...
for (omic_name in names(omic_dfs)) {
  ...
  models <- list(
    ...
    `Clinical + Combined (-c)`     = lmer_meta_all)
...
for (omic_name in names(omic_dfs)) {
  ...
  anova_labels <- anova_table %>%
    mutate(
      Compared_Model = recode(model_comparison,
        ...
        "lmer_meta_b vs lmer_meta_all"     = "Clinical + Combined (-c)"),
      Significance = case_when(
        `Pr(>Chisq)` < 0.001 ~ "***",
        `Pr(>Chisq)` < 0.01  ~ "**",
        `Pr(>Chisq)` < 0.05  ~ "*",
  ...
  custom_order <- c("Clinical", "Clinical + GRS", "Clinical + Pathway", 
                    ...
                    "Clinical + Metabolomics", "Clinical + Combined (-c)")
...
delta_labels <- c(
  ...
  "Clinical + Combined" = "+ Δ Combined (-c)")
...
gtsave(final_anova_table_clin_p2, filename = paste0(omic_out_dir, "/anova_tables_02/anova_table_clin_p.png"))
```
