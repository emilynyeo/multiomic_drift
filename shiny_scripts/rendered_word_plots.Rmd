...
# Read in SHAP value csvs for MERF 
...
shap_dfs <- lapply(names(shap_dfs), function(nm) {
  if (nm %in% names(rename_maps)) {
    shap_dfs[[nm]] %>%
      mutate(Feature = recode(Feature, !!!rename_maps[[nm]]))
  } else {
    shap_dfs[[nm]]
  }
...
shap_delta_dfs <- lapply(names(shap_delta_dfs), function(nm) {
  if (nm %in% names(rename_maps_delta)) {
    shap_delta_dfs[[nm]] %>%
      mutate(Feature = recode(Feature, !!!rename_maps_delta[[nm]]))
  } else {
    shap_delta_dfs[[nm]]
  }
...
for (name in names(shap_dfs)) {
  ...
  p <- plot_shap_beeswarm(df_top10, plot_title, high_color = "#C76374") + # setting to one col just for now
...
# Save large PDF
ggsave(
  filename = file.path(omic_out_dir, "all_shap_plots_combined_abs.pdf"),
...
## Repeat the above for SHAP delta 
title_map_delta <- c(
...
# Save large PDF
ggsave(
  filename = file.path(omic_out_dir, "all_delta_shap_plots_combined_abs.pdf"),
