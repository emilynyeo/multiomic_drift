...
source("/Users/emily/projects/research/Stanislawski/comps/mutli-omic-predictions/shiny_scripts/plot_input_data.R")
```
...
omic_out_dir <- "/Users/emily/projects/research/Stanislawski/comps/mutli-omic-predictions/paper_plots"
venn_dir <- "/Users/emily/projects/research/Stanislawski/comps/mutli-omic-predictions/paper_plots/"
```


```{r}
cluster_palette_contrast <- c(
  "#004E64", # deep teal blue
  "#B0E0E6", # powder blue
  "#4682B4", # steel blue
...
for (name in names(shap_dfs)) {
  ...
  # p <- plot_shap_beeswarm(df_top10, plot_title, high_color = high_col) +
  p <- plot_shap_beeswarm(df_top10, plot_title, high_color = "#C76374") + # 1 color for now
       theme(plot.margin = ggplot2::margin(25, 20, 25, 20))
...
for (name in names(shap_delta_dfs)) {
  ...
  p <- plot_shap_beeswarm(df_top10, plot_title, high_color = "#C76374") +
    theme(plot.margin = ggplot2::margin(25, 20, 25, 20))
...
for (omic_name in names(omic_dfs)) {
  ...
  anova_labels <- anova_table %>%
    dplyr::mutate(
      Compared_Model = dplyr::recode(model_comparison,
...
for (omic_name in names(all_r2_dfs)) {
  ...
  sig_plot <- ggplot(r2_annotated, aes(x = Model, y = R2m, fill = Model)) +
    geom_col(width = 0.7) +
    geom_text(aes(y = R2m + 0.03, label = Significance), size = 10) +
    scale_y_continuous(limits = c(0, 0.6), expand = expansion(mult = c(0, 0.1))) +
    scale_fill_manual(values = c(
      "Basic + Combined"      = "#b44241",
      "Basic + Metabolomics"   = "#C76374",
      "Basic + Micom"    = "#996759",
...
for (name in names(raw_tables)) {
  table_data <- raw_tables[[name]]
...
for (omic_name in names(omic_dfs)) {
  # Get R² results and model
  r2_df_clin_plus <- get(paste0(omic_name, "_r2_df"))
  lmer_meta_b <- get("lmer_meta_b")  # needed for AIC later
...
```{r anova_plot_generation}
for (omic_name in names(omic_dfs)) {
  r2_annotated <- get(paste0(omic_name, "_r2_annotated_clin_plus"))
  basic_r2 <- r2_annotated$`R²`[r2_annotated$Model == "Clinical"]

  sig_plot <- ggplot(r2_annotated, aes(x = Model, y = `R²`, fill = Model)) +
    geom_col(width = 0.7) +
    geom_text(aes(y = `R²` + 0.036, label = Significance), size = 12) +
    scale_y_continuous(limits = c(0, 0.72), expand = expansion(mult = c(0, 0.1))) +
    scale_fill_manual(values = c(
      "Clinical + Combined"      = "#b44241",
      "Clinical + Metabolomics"   = "#C76374",
...
# with % increase 
```{r}
# Function to add % R² Increase column *next to* R² column
add_r2_increase <- function(tbl) {
  base_r2 <- tbl$`R²`[1]
  r2_increase <- c(NA, round(100 * (tbl$`R²`[-1] - base_r2) / base_r2, 2))
  tbl <- tibble::add_column(tbl, `% R² Increase` = r2_increase, .after = "R²")
  return(tbl)
}
...
doc <- read_docx()
for (name in names(tables_clin_p_plus)) {
  table_data <- tables_clin_p_plus[[name]]
...
print(doc, target = paste0(omic_out_dir, "/anova_tables/anova_clinical_combined_with_increase.docx"))
```
...
ft_colors <- c(Clinical = "#fecdc7", Taxa = "#8B3A3A", 
               Micom = "#996759", Pathway = "#D8A39D", 
               Metabolomics  = "#C76374", 
               Combined = "#b44241", GRS = "#E18476",
               Basic = "#582F2B")

ft_colors_delta <- c(Clinical = "#fecdc7", Taxa = "#8B3A3A", 
                     Micom = "#996759", Pathway = "#D8A39D", 
                     Metabolomics  = "#C76374", 
                     Combined = "#b44241", GRS = "#E18476",
                     Basic = "#582F2B")
...
for (type in names(all_lass_ft_dfs)) {
  ...
  for (ft_name in names(ft_set)) {
    ...
    plot_feat <- top_features %>%
      ...
      slice_head(n = 10) %>%
      dplyr::filter(Estimate != 0) %>%   # <-- Remove rows with Estimate == 0
...
score_colors <- c(
  "Basic Score" = "#582F2B",
  "Clinical Score" = "#fecdc7",  
  "GRS Score" = "#E18476",
  "Taxa Score" = "#8B3A3A",
  "Micom Score" = "#996759",
  "Pathway Score" = "#D8A39D",
  "Metabolomics Score" = "#C76374",
  "Combined Omic Score" = "#b44241",
  "Actual BMI" = "grey50")
...
plot_score_means_delta <- function(df, title = "Mean Scores with Error Bars") {
  summary_df <- df %>%
  ...
  summary_df$`Score Type` <- factor(summary_df$`Score Type`, levels = c(
  ...
  # Plot using the summary_df
  ggplot(summary_df, aes(x = `Score Type`, y = Mean, fill = `Score Type`)) +
    geom_col() +
...
plot_score_means_long <- function(df, title = "Mean Scores with Error Bars") {
  summary_df <- df %>%
    select(`Basic Score`, `Clinical Score`, `GRS Score`, `Taxa Score`,
           `Micom Score`, `Pathway Score`, `Metabolomics Score`,  
           `Combined Omic Score`, `Actual BMI`) %>%
  ...
  summary_df$`Score Type` <- factor(summary_df$`Score Type`, levels = c(
  "Actual BMI", "Basic Score", "Clinical Score",
  "GRS Score", "Taxa Score", "Pathway Score",
  "Micom Score", "Metabolomics Score","Combined Omic Score"))
...
p10 <- plot_score_means_delta(merf_delta_bar, title = "MERF Delta: Mean Scores ± SE")
p20 <- plot_score_means_long(merf_long_bar, title = "MERF Long: Mean Scores ± SE")
p30 <- plot_score_means_delta(gl_lasso_delta_bar, title = "GLMM Delta: Mean Scores ± SE")
p40 <- plot_score_means_long(gl_lasso_long_bar, title = "GLMM Long: Mean Scores ± SE")
...
get_top_features <- function(df) {
  df %>%
    ...
    filter(cum_percent <= 0.5 | row_number() == 1) %>%  # always keep at least 1 feature
    pull(Feature)
}
...
# Output directory
venn_out_dir <- paste0(omic_out_dir, "venn_outputs")
