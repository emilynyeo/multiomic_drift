...
for (name in names(shap_dfs)) {
  ...
  p <- plot_shap_beeswarm(df_top10, plot_title, high_color = "#F33A6A") +
...
for (omic_name in names(omic_dfs)) {
  ...
  sig_plot <- ggplot(r2_annotated, aes(x = Model, y = `RÂ²`, fill = Model)) +
    geom_col(width = 0.7) +
    geom_text(aes(y = `RÂ²` + 0.036, label = Significance), size = 12) +
    scale_y_continuous(limits = c(0, 0.72), expand = expansion(mult = c(0, 0.1))) +
    scale_fill_manual(values = fill_vals) +
    scale_x_discrete(labels = label_set) +  
    labs(title = glue("RÂ² {omic_name_labels[[omic_name]]}"),
         x = NULL,
         y = expression("Marginal " ~ R^2)) +
...
... now trying with ggarrange

```{r}
library(ggpubr)

final_plot <- ggarrange(
  ggarrange(plotlist = mget(plot_names[c(1, 2)]), ncol = 2),
  ggarrange(plotlist = merf_plots[c(2, 3, 6, 7, 8)], ncol = 3),
  ggarrange(plotlist = lass_plots_lg[c(2, 7)], ncol = 2),
  ncol = 1,
  heights = c(1, 2, 1)
)
...
final_plot <- ggarrange(
  as_ggplot(ggarrange(plotlist = mget(plot_names[c(1, 2)]), ncol = 2)),
  as_ggplot(ggarrange(plotlist = merf_plots[c(2, 3, 6, 7, 8)], ncol = 3)),
  as_ggplot(ggarrange(plotlist = lass_plots_lg[c(2, 7)], ncol = 2)),
  ncol = 1,
  heights = c(1, 2, 1)
)
