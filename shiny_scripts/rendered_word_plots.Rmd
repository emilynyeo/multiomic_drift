...
for (omic_name in names(omic_dfs)) {
  ...
  # Save the sig plot
  ggsave(file.path(omic_out_dir, paste0("anova_tables/", omic_name, "_basic_sig_plot.png")), 
...
```{r}
## Testing all at once 
bl <- c(mget(plot_names[c(1,2)]), merf_plots[c(2, 3, 6, 7, 8)],  lass_plots_delta[c(2, 7)])
p <- ggdraw()
p <- p +
  draw_plot(bl[[1]], x = 0, y = 0.6+ 0.15, width = 0.48, height = 0.22) +
  draw_plot(bl[[2]], x = 0, y = 0.4+ 0.15, width = 0.48, height = 0.22) +
  draw_plot(bl[[8]], x = 0.01, y = 0.2+ 0.1, width = 0.48, height = 0.22) +   
  draw_plot(bl[[9]], x = 0.01, y = 0 + 0.04,   width = 0.48, height = 0.22) + 
  draw_plot(bl[[3]], x = 0.52, y = 0.76 + 0.04, width = 0.48, height = 0.18) + 
  draw_plot(bl[[4]], x = 0.52, y = 0.58 + 0.04, width = 0.48, height = 0.18) + 
  draw_plot(bl[[5]], x = 0.52, y = 0.4 + 0.04, width = 0.48, height = 0.18) + 
  draw_plot(bl[[6]], x = 0.52, y = 0.22 + 0.04, width = 0.48, height = 0.18) + 
  draw_plot(bl[[7]], x = 0.52, y = 0 + 0.04, width = 0.48, height = 0.18) + 
# Right side 
right_ys <- seq(0.7, 0, length.out = 5)  # y positions for 4 plots
for (i in 1:5) {
  p <- p + draw_plot(bl[[i+2]], x = 0.52, y = 0.05 + right_ys[i], width = 0.4, height = 0.18)
}
labels <- c("A.", "B.", "C.", "D.", "E.", "F.", "G.", "H.", "I.")
label_x <- c(0.01, 0.01, 0.01, 0.01, rep(0.516, 5))
label_y <- c(0.6+0.15+0.22, 
             0.4+0.15+0.22, 
             0.2+0.1+0.22, 
             0+0.05+0.22, 
             right_ys + 0.05 + 0.2)
...
ggsave(paste0(omic_out_dir, "aov_feats_combined/basic_long_test.png"), p, 
       width = 40, height = 40, dpi = 200, bg = 'white')
...
aov_basic_long_plt <- wrap_plots(basic_long, ncol = 1) + 
  theme(axis.title = element_text(size = 45),
    axis.text = element_text(size = 45),
    plot.margin = ggplot2::margin(20, 20, 20, 20, unit = "pt"))

aov_basic_long_plt
basic_lass_long_plot
basic_merf_long_plot

basic_long_cow <- ggdraw() +
  draw_plot(aov_basic_long_plt, x = 0.05, y = .55, width = .46, height = .45) +
  draw_plot(basic_lass_long_plot, x = .56, y = .55, width = .42, height = .45) +
  draw_plot(basic_merf_long_plot, x = 0.05, y = 0, width = .93, height = 0.55) +
  draw_plot_label(label = c("A.", "B.", "C."), size = 35,
                  x = c(0.03, 0.535, 0.03), y = c(1, 1, 0.55))

basic_long_cow <- ggdraw() +
  # Top-left plot
  draw_plot(aov_basic_long_plt,
            x = 0.08,
            y = 0.55 + 0.03,
            width = 0.445,
            height = 0.4) +
  # Top-right plot
  draw_plot(basic_lass_long_plot,
            x = 0.05 + 0.445 + 0.05,
            y = 0.55 + 0.03,
            width = 0.445,
            height = 0.4) +
  # Bottom plot (full width with margin)
  draw_plot(basic_merf_long_plot,
            x = 0.05,
            y = 0.04,
            width = 0.9,
            height = 0.5) +
...
ggsave(paste0(omic_out_dir, "aov_feats_combined/basic_long_ft_aov_combined_cow.png"), basic_long_cow, 
       width = 40, height = 40, dpi = 600, bg = 'white')

# *** MERF & GLLASSO Basic Delta ***
basic_delta <- mget(plot_names[c(3,4)])

aov_basic_delta_plt <- wrap_plots(basic_delta, ncol = 1) + 
                       theme(plot.margin = ggplot2::margin(20, 20, 20, 20, unit = "pt"))

aov_basic_delta_plt
basic_lass_delta_plot
basic_merf_delta_plot

basic_delta_cow <- ggdraw() +
  # Top-left plot
  draw_plot(aov_basic_delta_plt,
            x = 0.08,
            y = 0.55 + 0.03,
            width = 0.445,
            height = 0.4) +
  # Top-right plot
  draw_plot(basic_merf_delta_plot,
            x = 0.05 + 0.445,
            y = 0.05,
            width = 0.445,
            height = 0.92) +
  # Bottom plot (full width with margin)
  draw_plot(basic_lass_delta_plot,
            x = 0.05,
            y = 0.04,
            width = 0.445,
            height = 0.5) +
...
ggsave(paste0(omic_out_dir, "aov_feats_combined/basic_delta_ft_aov_combined_cow.png"), basic_delta_cow, 
       width = 40, height = 40, dpi = 200, bg = 'white')
...
aov_clin_long_plt <- wrap_plots(clin_long, ncol = 1) + 
                  theme(plot.margin = ggplot2::margin(20, 20, 20, 20, unit = "pt"))
...
clin_long_cow <- ggdraw() +
  # Top-left plot
  draw_plot(aov_clin_long_plt,
            x = 0.08,
            y = 0.5 + 0.1,
            width = 0.4,
            height = 0.38) +
  # top right plot
  draw_plot(clin_lass_long_plot,
            x = 0.45 + 0.05,
            y = 0.5 + 0.1,
            width = 0.4,
            height = 0.38) +
  # Bottom left (full width with margin)
  draw_plot(clin_merf_long_plot,
            x = 0.05,
            y = 0.17,
            width = 0.85,
            height = 0.4) +
...
ggsave(paste0(omic_out_dir, "aov_feats_combined/clinical_long_ft_aov_combined_cow.png"), clin_long_cow, 
       width = 40, height = 40, dpi = 600, bg = 'white')

# MERF & GLLASSO Clinical Delta
clin_delta <- mget(plot_names_clin_plus[c(3,4)])

aov_clin_delta_plt <- wrap_plots(clin_delta, ncol = 1) + 
                  theme(plot.margin = ggplot2::margin(20, 20, 20, 20, unit = "pt"))

aov_clin_delta_plt
clin_lass_delta_plot
clin_merf_delta_plot

clin_delta_cow <- ggdraw() +
  # Top-left plot
  draw_plot(aov_clin_delta_plt,
            x = 0.05,
            y = 0.55 + 0.05,
            width = 0.445,
            height = 0.37) +
  # Top-right plot
  draw_plot(clin_merf_delta_plot,
            x = 0.05 + 0.445,
            y = 0.25,
            width = 0.445,
            height = 0.73) +
  # Bottom plot (full width with margin)
  draw_plot(clin_lass_delta_plot,
            x = 0.06,
            y = 0.25,
            width = 0.37,
            height = 0.32) +
...
ggsave(paste0(omic_out_dir, "aov_feats_combined/clinical_delta_ft_aov_combined_cow.png"), clin_delta_cow, 
       width = 40, height = 40, dpi = 600, bg = 'white')
```
