...
imputed <- remove_columns(
  imputed_BL_6m,
  columns_to_remove = c("subject_id", "BMI (12m-BL)", "BMI (6m-BL)",
                        "Weight (12m-BL)","Weight (12m-6m)",
                        "Glucose (12m-BL)",
                        "HOMA-IR (12m-BL)", "Insulin (12m-BL)", "HDL (12m-BL)",
                        "LDL (12m-BL)", "Triglyceride lipid (12m-BL)",
...
m6_results <- train_and_save_models(
  ...
  "Weight (6m-BL)",
  train_control,
  "jan22_ALL_deltas_WT_BL_6m_g_ra_regr")

# describe the data
genus_ra_stats <- describe(BL_6m)

hist(genus_ra_stats$mean)
summary(genus_ra_stats$mean)

redundant_columns_genus <- names(BL_6m)[
  sapply(BL_6m, function(col) mean(col == 0, na.rm = TRUE) > 0.8) |
...
genus_results <- train_and_save_models(
  ...
  "Weight (6m-BL)",
  train_control,
  "jan22_ALL_deltas_WT_BL_6m_g_ra_no_redundant")
...
file_paths <- list(
  # genus
  deltas_WT_BL_6m_g_ra_regr_beta = "jan22_ALL_deltas_WT_BL_6m_g_ra_regr_beta.csv",
  deltas_WT_BL_6m_g_ra_regr_feature_importance = "jan22_ALL_deltas_WT_BL_6m_g_ra_regr_feature_importance.csv",
  deltas_WT_BL_6m_g_ra_regr_metrics = "jan22_ALL_deltas_WT_BL_6m_g_ra_regr_metrics.csv",
  # genus no redundant
  deltas_WT_BL_6m_g_ra_reg_no_redundant_beta = "jan22_ALL_deltas_WT_BL_6m_g_ra_no_redundant_beta.csv",
  deltas_WT_BL_6m_g_ra_reg_no_redundant_feature_importance = "jan22_ALL_deltas_WT_BL_6m_g_ra_no_redundant_feature_importance.csv",
  deltas_WT_BL_6m_g_ra_reg_no_redundant_metrics = "jan22_ALL_deltas_WT_BL_6m_g_ra_no_redundant_metrics.csv"
)
...
datasets <- list(
  "BL_6m_deltas_genus_ra" = list(
    beta = data_list$deltas_WT_BL_6m_g_ra_regr_beta,
    feature_importance = data_list$deltas_WT_BL_6m_g_ra_regr_feature_importance,
    metrics = data_list$deltas_WT_BL_6m_g_ra_regr_metrics
  ),
  "BL_6m_deltas_genus_ra_no_Redundant" = list(
    beta = data_list$deltas_WT_BL_6m_g_ra_reg_no_redundant_beta,
    feature_importance = data_list$deltas_WT_BL_6m_g_ra_reg_no_redundant_feature_importance,
    metrics = data_list$deltas_WT_BL_6m_g_ra_reg_no_redundant_metrics
  )
)
...
deltas_WT_BL_6m_g_ra_regr = list(
  beta = data_list$deltas_WT_BL_6m_g_ra_regr_beta,
  feature_importance = data_list$deltas_WT_BL_6m_g_ra_regr_feature_importance,
  metrics = data_list$deltas_WT_BL_6m_g_ra_regr_metrics)

deltas_WT_BL_6m_g_ra_reg_no_redundant = list(
  beta = data_list$deltas_WT_BL_6m_g_ra_reg_no_redundant_beta,
  feature_importance = data_list$deltas_WT_BL_6m_g_ra_reg_no_redundant_feature_importance,
  metrics = data_list$deltas_WT_BL_6m_g_ra_reg_no_redundant_metrics)

# Extract metrics and max RÂ² for all datasets
results_all <- extract_metrics(deltas_WT_BL_6m_g_ra_regr)
results_allno_re <- extract_metrics(deltas_WT_BL_6m_g_ra_reg_no_redundant)
...
create_feature_plot(
  ...
  "drift_fs/figures/deltas/all_jan22/jan22_WT_all_deltas_BL_6m_all_g_feature_plot.pdf")

create_feature_plot(
  all_omic_genus_no_rendundant_features,
  "Top 10 Features - Non Redundant Genus + All omics",
  "drift_fs/figures/deltas/all_jan22/jan22_WT_all_deltas_BL_6m_all_g_no_re_feature_plot.pdf")
...
pdf("drift_fs/figures/deltas/all_jan22/jan22_WT_all_deltas_BL_6m_all_g_6m.pdf", width = 7, height = 7)
...
pdf("drift_fs/figures/deltas/all_jan22/jan22_WT_all_deltas_BL_6m_all_g_no_re_6m.pdf", width = 7, height = 7)
...
top_models_list_all_omic_m6 <- deltas_WT_BL_6m_g_ra_regr$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)

top_models_list_all_omic_m6_no_redundant <- deltas_WT_BL_6m_g_ra_reg_no_redundant$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)
