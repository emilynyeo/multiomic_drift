...
all_delta_BL_6m <- read_csv(paste0(data_dir, "jan22_all_delta_BL_6m.csv"))
all_delta_BL_12m <- read_csv(paste0(data_dir, "jan22_all_delta_BL_12m.csv"))
all_delta_6m_12m <- read_csv(paste0(data_dir, "jan22_all_delta_6m_12m.csv"))
...
head(colnames(BL_12m), 30)

imputed <- remove_columns(
  imputed_BL_12m,
  columns_to_remove = c("subject_id", "BMI (6m-BL)",
                        "Weight (6m-BL)", "Weight (12m-BL)","Weight (12m-6m)",
                        "Glucose (6m-BL)",
                        "HOMA-IR (6m-BL)", "Insulin (6m-BL)", "HDL (6m-BL)",
                        "LDL (6m-BL)", "Triglyceride lipid (6m-BL)",
                        "outcome_wt_fnl_BL","outcome_wt_fnl_6m", 
                        "outcome_wt_fnl_12m","outcome_wt_fnl_BL_12m",
                        "outcome_BMI_fnl_BL", "outcome_BMI_fnl_6m", 
                        "outcome_BMI_fnl_12m", "outcome_BMI_fnl_BL_12m"))
...
m6_results <- train_and_save_models(
  ...
  "BMI (12m-BL)",
  train_control,
  "jan22_ALL_deltas_BMI_BL_12m_g_ra_regr")

# describe the data
genus_ra_stats <- describe(BL_12m)

hist(genus_ra_stats$mean)
summary(genus_ra_stats$mean)

redundant_columns_genus <- names(BL_12m)[
  sapply(BL_12m, function(col) mean(col == 0, na.rm = TRUE) > 0.8) |
...
genus_results <- train_and_save_models(
  ...
  "BMI (12m-BL)",
  train_control,
  "jan22_ALL_deltas_BMI_BL_12m_g_ra_no_redundant")
...
file_paths <- list(
  # genus
  deltas_BMI_BL_12m_g_ra_regr_beta = "jan22_ALL_deltas_BMI_BL_12m_g_ra_regr_beta.csv",
  deltas_BMI_BL_12m_g_ra_regr_feature_importance = "jan22_ALL_deltas_BMI_BL_12m_g_ra_regr_feature_importance.csv",
  deltas_BMI_BL_12m_g_ra_regr_metrics = "jan22_ALL_deltas_BMI_BL_12m_g_ra_regr_metrics.csv",
  # genus no redundant
  deltas_BMI_BL_12m_g_ra_reg_no_redundant_beta = "jan22_ALL_deltas_BMI_BL_12m_g_ra_no_redundant_beta.csv",
  deltas_BMI_BL_12m_g_ra_reg_no_redundant_feature_importance = "jan22_ALL_deltas_BMI_BL_12m_g_ra_no_redundant_feature_importance.csv",
  deltas_BMI_BL_12m_g_ra_reg_no_redundant_metrics = "jan22_ALL_deltas_BMI_BL_12m_g_ra_no_redundant_metrics.csv"
)
...
datasets <- list(
  "BL_12m_deltas_genus_ra" = list(
    beta = data_list$deltas_BMI_BL_12m_g_ra_regr_beta,
    feature_importance = data_list$deltas_BMI_BL_12m_g_ra_regr_feature_importance,
    metrics = data_list$deltas_BMI_BL_12m_g_ra_regr_metrics
  ),
  "BL_12m_deltas_genus_ra_no_Redundant" = list(
    beta = data_list$deltas_BMI_BL_12m_g_ra_reg_no_redundant_beta,
    feature_importance = data_list$deltas_BMI_BL_12m_g_ra_reg_no_redundant_feature_importance,
    metrics = data_list$deltas_BMI_BL_12m_g_ra_reg_no_redundant_metrics
  )
)
...
deltas_BMI_BL_12m_g_ra_regr = list(
  beta = data_list$deltas_BMI_BL_12m_g_ra_regr_beta,
  feature_importance = data_list$deltas_BMI_BL_12m_g_ra_regr_feature_importance,
  metrics = data_list$deltas_BMI_BL_12m_g_ra_regr_metrics)

deltas_BMI_BL_12m_g_ra_reg_no_redundant = list(
  beta = data_list$deltas_BMI_BL_12m_g_ra_reg_no_redundant_beta,
  feature_importance = data_list$deltas_BMI_BL_12m_g_ra_reg_no_redundant_feature_importance,
  metrics = data_list$deltas_BMI_BL_12m_g_ra_reg_no_redundant_metrics)

# Extract metrics and max RÂ² for all datasets
results_all <- extract_metrics(deltas_BMI_BL_12m_g_ra_regr)
results_allno_re <- extract_metrics(deltas_BMI_BL_12m_g_ra_reg_no_redundant)
...
create_feature_plot(
  ...
  "drift_fs/figures/deltas/all_jan22/jan22_all_deltas_BL_12m_all_g_feature_plot.pdf")

create_feature_plot(
  all_omic_genus_no_rendundant_features,
  "Top 10 Features - Non Redundant Genus + All omics",
  "drift_fs/figures/deltas/all_jan22/jan22_all_deltas_BL_12m_all_g_no_re_feature_plot.pdf")
...
pdf("drift_fs/figures/deltas/all_jan22/jan22_all_deltas_BL_12m_all_g_12m.pdf", width = 7, height = 7)
...
pdf("drift_fs/figures/deltas/all_jan22/jan22_all_deltas_BL_12m_all_g_no_re_12m.pdf", width = 7, height = 7)
...
top_models_list_all_omic_12m <- deltas_BMI_BL_12m_g_ra_regr$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)

top_models_list_all_omic_12m_no_redundant <- deltas_BMI_BL_12m_g_ra_reg_no_redundant$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)
