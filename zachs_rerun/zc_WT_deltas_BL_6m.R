...
imputed <- remove_columns(
  imputed_BL_6m,
  columns_to_remove = c("subject_id", "SampleID", "BMI (6m-BL)"))
...
genus_results <- train_and_save_models(
  ...
  "Weight (6m-BL)",
  train_control,
  "deltas_WT_BL_6m_g_ra_reg_no_redundant")
...
file_paths <- list(
  # genus
  deltas_WT_BL_6m_g_ra_regr_beta = "deltas_WT_BL_6m_g_ra_regr_beta.csv",
  deltas_WT_BL_6m_g_ra_regr_feature_importance = "deltas_WT_BL_6m_g_ra_regr_feature_importance.csv",
  deltas_WT_BL_6m_g_ra_regr_metrics = "deltas_WT_BL_6m_g_ra_regr_metrics.csv",
  # genus no redundant
  deltas_WT_BL_6m_g_ra_reg_no_redundant_beta = "deltas_WT_BL_6m_g_ra_reg_no_redundant_beta.csv",
  deltas_WT_BL_6m_g_ra_reg_no_redundant_feature_importance = "deltas_WT_BL_6m_g_ra_reg_no_redundant_feature_importance.csv",
  deltas_WT_BL_6m_g_ra_reg_no_redundant_metrics = "deltas_WT_BL_6m_g_ra_reg_no_redundant_metrics.csv"
)
...
create_feature_plot(
  ...
  "drift_fs/figures/deltas/jan22_deltas_WT_BL_6m_all_g_feature_plot.pdf")

create_feature_plot(
  all_omic_genus_no_rendundant_features,
  "Top 10 Features - Non Redundant Genus + All omics",
  "drift_fs/figures/deltas/jan22_deltas_WT_BL_6m_all_g_no_re_feature_plot.pdf")
...
pdf("drift_fs/figures/deltas/jan22_deltas_WT_BL_6m_all_g.pdf", width = 7, height = 7)
...
pdf("drift_fs/figures/deltas/jan22_deltas_WT_BL_6m_all_g_no_re.pdf", width = 7, height = 7)
...
top_models_list_all_omic_6m <- deltas_WT_BL_6m_g_ra_regr$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)

top_models_list_all_omic_6m_no_redundant <- deltas_WT_BL_6m_g_ra_reg_no_redundant$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)
...
print(top_models_list_all_omic_6m)
print(top_models_list_all_omic_6m_no_redundant)
