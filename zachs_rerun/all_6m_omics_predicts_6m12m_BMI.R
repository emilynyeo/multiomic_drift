...
all$bmi_6m_12m <- all$outcome_BMI_fnl_12m - all$outcome_BMI_fnl_6m
#all <- all[!is.na(all$bmi_6m_12m), ] # Remove rows with missing values in bmi_6m_12m
...
m6_results <- train_and_save_models(
  ...
  "bmi_6m_12m",
  train_control,
  "bmi_6m_12m_BL_train_omic_g_ra_regression")
...
genus_results <- train_and_save_models(
  ...
  "bmi_6m_12m",
  train_control,
  "bmi_6m_12m_BL_train_omic_g_regression_no_redundant")
...
file_paths <- list(
  # genus
  bmi_6m_12m_BL_omic_g_ra_regression_beta = "bmi_6m_12m_BL_train_omic_g_ra_regression_beta.csv",
  bmi_6m_12m_BL_omic_g_ra_regression_feature_importance = "bmi_6m_12m_BL_train_omic_g_ra_regression_feature_importance.csv",
  bmi_6m_12m_BL_omic_g_ra_regression_metrics = "bmi_6m_12m_BL_train_omic_g_ra_regression_metrics.csv",
  # genus no redundant
  bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_beta = "bmi_6m_12m_BL_train_omic_g_regression_no_redundant_beta.csv",
  bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_feature_importance = "bmi_6m_12m_BL_train_omic_g_regression_no_redundant_feature_importance.csv",
  bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_metrics = "bmi_6m_12m_BL_train_omic_g_regression_no_redundant_metrics.csv"
)
...
datasets <- list(
  "bmi_6m_12m_BL_omics_Genus_ra" = list(
    beta = data_list$bmi_6m_12m_BL_omic_g_ra_regression_beta,
    feature_importance = data_list$bmi_6m_12m_BL_omic_g_ra_regression_feature_importance,
    metrics = data_list$bmi_6m_12m_BL_omic_g_ra_regression_metrics
  ),
  "bmi_6m_12m_BL_omics_Genus_ra_No_Redundant)" = list(
    beta = data_list$bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_beta,
    feature_importance = data_list$bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_feature_importance,
    metrics = data_list$bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_metrics
  )
)
...
top_20_features <- get_top_n_features_all_models(data_list$bmi_6m_12m_BL_omic_g_ra_regression_feature_importance, 20)
print(top_20_features)

top_20_features_no_r <- get_top_n_features_all_models(data_list$bmi_6m_12m_BL_omic_g_ra_regression_no_redundant_feature_importance, 20)
...
results_all <- extract_metrics(bmi_6m_12m_BL_omics_Genus_ra)
results_allno_re <- extract_metrics(bmi_6m_12m_BL_omics_Genus_ra_No_Redundant)
...
genus_titles <- c("6m Omic predicting 6-12m BMI change - Model Testing RÂ²")
genus_titles_no_redundant <- c("6m Omic predicting 6-12m BMI change - Non Redundant")

# Generate combined genus plot
all_omic_plot_genus_ra <- create_plots(all_g_data_list, max_r2, genus_titles)
pdf("drift_fs/figures/6m_omic_predicting_6m12m_BMI/feb18_BLomic_BL6mBMI_train_g_data_list.pdf", width = 7, height = 7)
...
pdf("drift_fs/figures/6m_omic_predicting_6m12m_BMI/feb18_BLomic_BL6mBMI_train_g_data_list_no_re.pdf", width = 7, height = 7)
...
all_omic_genus_no_rendundant_features <- extract_top_features(datasets$`bmi_6m_12m_BL_omics_Genus_ra_No_Redundant)`)
all_omic_genus_features <- extract_top_features(datasets$bmi_6m_12m_BL_omics_Genus_ra)
...
create_feature_plot(
  ...
  "Top FI's - 6m omics predicting 6m-12m BMI change",
  "drift_fs/figures/6m_omic_predicting_6m12m_BMI/feb18_6momic_6m12mBMI_train_g_feature_plot.pdf")

create_feature_plot(
  all_omic_genus_no_rendundant_features,
  "Top FI's - 6m omics predicting 6m-12m BMI change (non redundant)",
  "drift_fs/figures/6m_omic_predicting_6m12m_BMI/feb18_6momic_6m12mBMI_train_g_no_rendundant_feature_plot.pdf")
...
top_models_list_all_omic_m12 <- bmi_6m_12m_BL_omics_Genus_ra$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)

top_models_list_all_omic_m12_no_redundant <- bmi_6m_12m_BL_omics_Genus_ra_No_Redundant$metrics %>%
  filter(DataType == "Test") %>%
  arrange(desc(R2)) %>%
  slice_head(n = 3) %>%
  pull(Model)
