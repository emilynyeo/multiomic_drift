...
meta_slim <- meta_data_df %>% 
  dplyr::select(c("subject_id", "randomized_group", "score_std", 
                  ...
                  "outcome_wt_fnl_6m", "Glucose_BL", "HOMA_IR_BL",
...
write.csv(g_ra_all_outer, 
          paste0(save_dir, 
                 "jan22_wt_6m_genus_ra_all_omics_outer.csv"), 
          row.names = FALSE)

write.csv(g_ra_all_inner, 
          paste0(save_dir, 
                 "jan22_wt_6m_genus_ra_all_omics_inner.csv"), 
          row.names = FALSE)
...
omic_g_ra_outer <- read_csv(paste0(data_dir, "jan22_wt_6m_genus_ra_all_omics_outer.csv"))
omic_g_ra_inner <- read_csv(paste0(data_dir, "jan22_wt_6m_genus_ra_all_omics_inner.csv"))
...
### Process DFs 
imputed_m6 <- preprocess_data(m6, 
                              latent_variables_6m, 
                              "medianImpute")

# remove outcome_BMI_fnl_BL from the genus and species dataframes
tail(colnames(imputed_m6), 300)
imputed <- remove_columns(imputed_m6, 
                          columns_to_remove = c("subject_id", "SampleID", "outcome_BMI_fnl_6m"))
...
m6_results <- train_and_save_models(
  ...
  "outcome_wt_fnl_6m",
  train_control,
  "m6_wt_all_omic_g_ra_regression")
...
genus_results <- train_and_save_models(
  ...
  "outcome_wt_fnl_6m",
  train_control,
  "m6_wt_all_omic_g_regression_no_redundant")
...
file_paths <- list(
  # genus
  m6_all_omic_g_ra_regression_beta = "m6_wt_all_omic_g_ra_regression_beta.csv",
  m6_all_omic_g_ra_regression_feature_importance = "m6_wt_all_omic_g_ra_regression_feature_importance.csv",
  m6_all_omic_g_ra_regression_metrics = "m6_wt_all_omic_g_ra_regression_metrics.csv",
  
  # genus no redundant
  m6_all_omic_g_ra_regression_no_redundant_beta = "m6_wt_all_omic_g_regression_no_redundant_beta.csv",
  m6_all_omic_g_ra_regression_no_redundant_feature_importance = "m6_wt_all_omic_g_regression_no_redundant_feature_importance.csv",
  m6_all_omic_g_ra_regression_no_redundant_metrics = "m6_wt_all_omic_g_regression_no_redundant_metrics.csv"
)
...
create_feature_plot(
  ...
  "Top 10 - WT, Non Redundant Genus + All omics",
  "drift_fs/figures/all_omics_genus_ra/jan22_WT_all_m6_genus_feature_plot.pdf")

create_feature_plot(
  all_omic_genus_no_rendundant_features,
  "Top 10 - WT, Non Redundant Genus + All omics",
  "drift_fs/figures/all_omics_genus_ra/jan22_WT_all_m6_genus_no_rendundant_feature_plot.pdf")
