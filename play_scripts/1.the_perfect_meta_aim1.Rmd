...
I was just looking at variable correlatons, variations, summary stats and PCAs

## Looking at correlations between vars
```{r echo=FALSE, warning=FALSE}
scatterplotMatrix(a1_meta[3:5])
```

```{r message=FALSE, warning=FALSE}
scatterplotMatrix(a1_meta[10:15])
```
...
regVar <- c("age", "income", "HOMA_IR_BL", "spk_EE_int_kcal_day_BL",
            ...
            "Cholesterol_lipid_BL", "Triglyceride_lipid_BL" ,
            "HDL_Total_Direct_lipid_BL", "LDL_Calculated_BL")
subset_df <- a1_meta %>% select(all_of(regVar))
str(subset_df)
...
```{r}
mosthighlycorrelated(a1_meta[2:31], 10)
```
...
variation in socio variables 
```{r}
makeProfilePlot(socio_list, socio)
```

I probably need to scale all my variables before this'll make any sense

variation in some blood markers and bloop pressure
```{r}
makeProfilePlot(blood_bp_list, blood_bp)
```

variation in hunger-associated variables
```{r}
makeProfilePlot(hunger_list, hunger)
```

variation in biometric variables
```{r}
makeProfilePlot(body_list, body)
```

variations in energy list
```{r}
makeProfilePlot(energy_list, energy)
```
...
Mean and Std. by sex
```{r}
printMeanAndSdByGroup(a1_meta[2:31],a1_meta[4])
```
...
Thus, its better to first standardise the variables so that they all have variance 1 and mean 0, and to then carry out the principal component analysis on the standardised data. This would allow us to find the principal components that provide the best low-dimensional representation of the variation in the original data, without being overly biased by those variables that show the most variance in the original data."

You can standardise variables in R using the scale function.
```{r}
scaled_a1 <- as.data.frame(scale(a1_meta[2:31]))
head(scaled_a1)
#scaled_a1 <- as.data.frame(scale(a1_meta[, c(3, 5:10, 15:31)]))
```

should this be done to my zero 1 category variables?
...
```{r}
comboInfo <- findLinearCombos(noNA_scaled_a1)
comboInfo
```
...
training_big_nos <- training %>% 
                dplyr::select(c(rmr_kcald_BL,spk_EE_int_kcal_day_BL,
                                #WBTOT_FAT_BL, WBTOT_LEANmass_BL, 
                                height_cm,
                                wc_average_BL, avg_systolic_BL,
                                Ghrelin_BL, LDL_Calculated_BL, Triglyceride_lipid_BL,
                                Cholesterol_lipid_BL, Peptide_YY_BL))
training_small <- training %>% 
                dplyr::select(-c(record_id, rmr_kcald_BL,spk_EE_int_kcal_day_BL,
                                 WBTOT_FAT_BL, WBTOT_LEANmass_BL, height_cm,
                                wc_average_BL, avg_systolic_BL,
                                Ghrelin_BL, LDL_Calculated_BL, Triglyceride_lipid_BL,
                                Cholesterol_lipid_BL, Peptide_YY_BL))

# Combine original and transformed data for easy comparison
original_melted_big <- melt(training_big_nos)
original_melted_small <- melt(training_small)
transformed_melted <- melt(train_Transformed)

# Plot histograms
ggplot(original_melted_big, aes(x = value, fill = variable)) + 
    geom_histogram(bins = 30, alpha = 0.5) + 
    ggtitle("Histograms of Original Training Data with bigger numbers") +
    theme_minimal()

ggplot(original_melted_small, aes(x = value, fill = variable)) + 
    geom_histogram(bins = 30, alpha = 0.5) + 
    ggtitle("Histograms of Original Training Data with smaller numbers") +
    theme_minimal()
...
```{r}
# Density plots
ggplot(original_melted_big, aes(x = value, color = variable)) +
    geom_density() +
    ggtitle("Density Plots of Original Training Data with bigger numbers") +
    theme_minimal()

ggplot(original_melted_small, aes(x = value, color = variable)) +
    geom_density() +
    ggtitle("Density Plots of Original Training Data that's smaller") +
    theme_minimal()
...
```{r}
# Box plots
ggplot(original_melted_big, aes(x = variable, y = value, fill = variable)) +
    geom_boxplot() +
    ggtitle("Box Plots of Original Training Data with bigger numbers") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(original_melted_small, aes(x = variable, y = value, fill = variable)) +
    geom_boxplot() +
    ggtitle("Box Plots of Original Training Data that's smaller") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Socio transformed 
```{r}
makeProfilePlot(socio_list_t, socio_t)
```

blood transformed variable variation 
```{r}
makeProfilePlot(blood_bp_list_t, blood_bp_t)
```

sugar transformed variable variation 
```{r}
makeProfilePlot(sugar_list_t, sugar_t)
```

hunger transformed variable variation 
```{r}
makeProfilePlot(hunger_list_t, hunger_t)
```

body transformed variable variation 
```{r}
makeProfilePlot(body_list_t, body_t)
```

energy transformed variable variation 
```{r}
makeProfilePlot(energy_list_t, energy_t)
```
...
```{r}
printMeanAndSdByGroup(train_Transformed[2:31],train_Transformed[4])
```

## Correlation when transformed 

```{r}
mosthighlycorrelated(train_Transformed[2:31], 10) # top 10 most correlated variables
```
...
```{r}
trn_ctrl <- trainControl(method = "cv", # cross val
                         #number = "10", # 10 fold
...
Here I using BL BMI as my outcome variable 
...
train(
  ...
  metric = "RMSE",
  maximize = FALSE,
  trControl = trainControl(trn_ctrl),
  tuneGrid = NULL,
