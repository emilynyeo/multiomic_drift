...
basic <- c('subject_id','outcome_BMI_fnl', 'time','age', 'sex')
meta_keep <- c('subject_id', 'outcome_BMI_fnl', 'time', 'randomized_group', 
               'cohort_number', 'sex', 'race', 'age', 'Hemoglobin_A1C', 'HDL_Total_Direct_lipid', 
               'HOMA_IR', 'Insulin_endo', 'LDL_Calculated', 'Glucose.x')
only_grs <- c('subject_id','outcome_BMI_fnl', 'time','bmi_prs')
only_taxa <- c('subject_id', 'outcome_BMI_fnl', 'time', grep("^g__", names(long), value = TRUE))

micom_start <- which(names(long) == "Diacetyl")
micom_end <- which(names(long) == "aldehydo.D.xylose")
only_micom <- c('subject_id','outcome_BMI_fnl', 'time', names(long)[micom_start:micom_end])
...
only_pathway <- c('subject_id', 'outcome_BMI_fnl', 'time', names(long)[path_start:path_end])

tabo_start <- which(names(long) == "non_HDL_C")
tabo_end <- which(names(long) == "IDL_TG_pct")
only_tabo <- c('subject_id', 'outcome_BMI_fnl', 'time', names(long)[tabo_start:tabo_end])
...
for (df in data_frames) {
  ...
  BL_results <- train_and_save_models(
    train_set, "outcome_BMI_fnl", train_control,
    paste0("BL_pred_BL_bmi_", df))
  
  models[[df]] <- BL_results # store or print
}
...
for (df_name in names(df_names)) {
  ...
  file_paths <- list(
    beta = paste0("BL_pred_BL_bmi_", df_name, "_beta.csv"),
    feature_importance = paste0("BL_pred_BL_bmi_", df_name,"_feature_importance.csv"),
    metrics = paste0("BL_pred_BL_bmi_", df_name, "_metrics.csv"))
  ...
  titles <- c(paste0(df_name, " BL omic predicting BL BMI - Model Testing R2"))
  
  # Generate combined genus plot
  omic_plot_BL_bmi <- create_plots(all_data_list, max_r2, titles)
  pdf(file = file.path(base_path, 
                       paste0("figures/BL_pred_BL_bmi_", df_name, ".pdf")), 
      width = 7, height = 7)
  print(omic_plot_BL_bmi)
  dev.off()
}
...
for (omic_name in names(all_predictions)) {
  ...
  if (!all(c("lasso_model", "rf_model", "ridge_model") %in% names(preds))) next
  lasso <- preds$lasso_model
  rf <- preds$rf_model
  ...
  test_subset <- test_data[test_data$pred_id %in% ids, 
                           c("pred_id", "outcome_BMI_fnl", "subject_id")]
...
#### ANOVA for RF 
basic_rf <- lm(outcome_BMI_fnl ~ prediction_basic, data = rf)
meta_rf <- lm(outcome_BMI_fnl ~ prediction_basic + prediction_meta, data = rf)
grs_rf <- lm(outcome_BMI_fnl ~ prediction_basic + prediction_grs, data = rf)
taxa_rf <- lm(outcome_BMI_fnl ~ prediction_basic + prediction_taxa, data = rf)
pathway_rf <- lm(outcome_BMI_fnl ~ prediction_basic + prediction_pathway, data = rf)
micom_rf <- lm(outcome_BMI_fnl ~ prediction_basic + prediction_micom, data = rf)
metabo_rf <- lm(outcome_BMI_fnl ~ prediction_basic + prediction_metabo, data = rf)
...
# Step 2: Reorder factor levels (Basic first, then descending R²)
ordered_levels <- c("Basic", plot_data_long %>%
                      dplyr::filter(model != "Basic") %>%
                      arrange(desc(adj_r2)) %>%
                      pull(model))

plot_data_long <- plot_data_long %>%
  dplyr::mutate(model = factor(model, levels = ordered_levels))
...
rf_numeric <- rf %>% dplyr::select(starts_with("prediction_")) %>% as.matrix()
heatmap(rf_numeric, scale = "none", col = heat.colors(256))

library(pheatmap)
pheatmap(rf_numeric,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         scale = "none",  # use "row" to normalize each subject
         show_rownames = TRUE,
         show_colnames = TRUE,
...
df_numeric <- BL[12:ncol(BL)][, sapply(BL[12:ncol(BL)], is.numeric)]
df_numeric <- pathway[4:ncol(pathway)][, sapply(pathway[4:ncol(pathway)], is.numeric)]
pca <- prcomp(df_numeric, scale. = TRUE)
# Plot first 2 principal components
biplot(pca, scale = 0)

library(factoextra)
fviz_pca_biplot(pca, repel = TRUE, geom = c("point"),
                label = "ind", invisible = c("ind", "ind.sup"), 
                addEllipses=TRUE, ellipse.level=0.95,)

### Cluster
dist_matrix <- as.dist(1 - abs(cor(df_numeric)))
...
### 
plot(long$bmi_prs, long$outcome_BMI_fnl)
cor(long$bmi_prs, long$outcome_BMI_fnl)
plot(BL$bmi_prs, BL$bmi_0_6)
cor(BL$bmi_prs, BL$bmi_0_6)
