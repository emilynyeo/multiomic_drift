...
out_dir <- "mkdir /Users/emily/projects/research/Stanislawski/comps/mutli-omic-predictions/play_scripts/2.models/glmmlasso/march30_long"
...
long <- read.csv(file.path(long_dir, 'all_processing_long.csv')) %>% 
  ...
  dplyr::mutate(time = as.factor(time),
    ...
    randomized_group = as.numeric(train_slim$randomized_group),
    sex = as.numeric(sex),
...
data_frames <- c("train_basic", "train_meta", "train_micom", "train_pathway", "train_taxa")
# Loop to process each data frame
for (df in data_frames) {
  df_data <- get(df)
  df_data <- df_data %>% filter(!subject_id %in% missing_subjects)
