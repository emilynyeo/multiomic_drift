...
out_dir <- "/Users/emily/projects/research/Stanislawski/comps/mutli-omic-predictions/play_scripts/2.models/glmmlasso/"
...
train_slim <- train %>% dplyr::select(-c("Unnamed..0_y", "Unnamed..0_x", "Unnamed..0_train_long", 
                                         "Unnamed..0_merged_data", "sample_id", "subject_id",
...
test_slim <- test %>% 
  dplyr::select(-c("Unnamed..0_y", "Unnamed..0_x", "Unnamed..0_test_long", 
                   "Unnamed..0_merged_data", "sample_id", "subject_id",
                   "all_samples_x", "all_samples_y", "record_id", "SampleID", 
                   "time_x", "time_y", "x_t", "t", "outcome_BMI_fnl_x", 
                   "outcome_BMI_fnl_y", "outcome_BMI_fnl_merged_data", 
...
train_slim <- train_slim %>% rename(subject_id = all_samples,
                                    BMI = outcome_BMI_fnl_train_long,
                                    range = time,
                                    homo_ir = HOMA_IR,
...
ggsave(paste0(out_dir,"long_lambda_elbow_only_meta_feb14.png"), 
...
file_path <- file.path(out_dir, "long_meta_predictions_feb14.csv")
...
ggplot() +
  ...
  xlab("Penalty Coefficient (Lambda)") +
  ylab("Number of Included Variables")
ggsave(paste0(out_dir,"long_lambda_elbow_only_taxa_feb14.png"), 
       width=6, height=4, units="in", dpi=320)
...
pred_df_taxa <- as.data.frame(cbind(test_taxa_filtered$subject_id,
                                    ...
                                    scale(pred_risk_scores_taxa))); colnames(pred_df_taxa) <- c("subject_id",
                                                                                                ...
                                                                                                "actual",
                                                                                                "predicted_taxa")
file_path <- file.path(out_dir, "long_taxa_predictions_feb14.csv")
...
ggplot() +
  ...
  xlab("Penalty Coefficient (Lambda)") +
  ylab("Number of Included Variables")
ggsave(paste0(out_dir,"long_lambda_elbow_only_micom_feb14.png"), 
       width=6, height=4, units="in", dpi=320)
...
pred_df_micom<- as.data.frame(cbind(test_micom_filtered$subject_id,
                                    ...
                                    scale(pred_risk_scores_micom))); colnames(pred_df_micom) <- c("subject_id",
                                                                                                  ...
                                                                                                  "actual",
                                                                                                  "predicted_micom")
file_path <- file.path(out_dir, "long_micom_predictions_feb14.csv")
...
ggplot() +
  ...
  xlab("Penalty Coefficient (Lambda)") +
  ylab("Number of Included Variables")
ggsave(paste0(out_dir,"long_lambda_elbow_only_pathway_feb14.png"), 
       width=6, height=4, units="in", dpi=320)
...
pred_df_path<- as.data.frame(cbind(test_path_filtered$subject_id,
                                   ...
                                   scale(pred_risk_scores_path))); colnames(pred_df_path) <- c("subject_id",
                                                                                               ...
                                                                                               "actual",
                                                                                               "predicted_pathway")
file_path <- file.path(out_dir, "long_path_predictions_feb14.csv")
