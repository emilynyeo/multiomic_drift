...
train_set <- long[long[["subject_id.x"]] %in% train_names, ]  # Select rows where ID_VAR is in train_names
test_set <- long[long[["subject_id.x"]] %in% test_names, ]    # Select rows where ID_VAR is in test_names

test <- test_set %>% dplyr::select(c(subject_id.x, outcome_BMI_fnl, time))
