...
colnames(path_no_nzv_BL) <- gsub(" ", "_", colnames(path_no_nzv_all))
all_path_relative_abundance <- path_no_nzv_all %>%
  #rownames_to_column(var = "subject_id") %>%  # Convert row names to a column
  dplyr::rowwise() %>%
  dplyr::mutate(total = sum(c_across(2:327), na.rm = TRUE)) %>%
  dplyr::mutate(across(2:327, ~ .x / total)) %>%
  dplyr::select(-total)# %>%
  #column_to_rownames(var = "SampleID")

# Replace spaces with underscores in column names
colnames(path_no_nzv_BL) <- gsub(" ", "_", colnames(path_no_nzv_BL))
bl_path_relative_abundance <- path_no_nzv_BL %>%
  #rownames_to_column(var = "subject_id") %>%  # Convert row names to a column
  dplyr::rowwise() %>%
  dplyr::mutate(total = sum(c_across(2:327), na.rm = TRUE)) %>%
  dplyr::mutate(across(2:327, ~ .x / total)) %>%
